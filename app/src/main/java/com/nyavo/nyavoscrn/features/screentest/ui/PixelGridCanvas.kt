...
import com.nyavo.nyavoscrn.core.designsystem.theme.VioletPalette
    ...
    Canvas(
        ...
        zones.forEach { zone ->
            ...
            val overlayColor: Color? = when {
                zone.id == activeZoneId -> ZoneActiveGlow.copy(alpha = pulseAlpha * 0.5f)
                zone.isFalsePositive -> ZoneFalsePositiveColor.copy(alpha = 0.55f)
...
private fun createPixelGridBitmap(
    ...
    for (row in 0 until rows) {
        for (col in 0 until cols) {
            ...
            val baseColor = VioletPalette[(row + col) % VioletPalette.size]
            val lightColor = baseColor.lighten(0.25f)
            val darkColor = baseColor.darken(0.35f)
